...
pequiv <- read_spss("data/SOEP/SPSS_EN/pequiv.sav")
health <- read_spss("data/SOEP/SPSS_EN/health.sav")
hbrutt <- read_spss("data/SOEP/SPSS_EN/hbrutt.sav")
...
pequiv_1 <- pequiv %>% 
  select(cid, hid, pid,syear, i11102, d11106, p11101) %>% 
  filter(!(i11102 < 0))


#Select MCS value in health. Additionally remove any mcs values which are 
#negative or na
health_1 <- health %>% 
  select(cid, pid, syear, mcs) %>% 
  filter(!mcs < 0) %>% 
  filter(!is.na(mcs))
...
merge_2002 <- full_join(pequiv_2002, health_2002, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2002) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2004 <- full_join(pequiv_2004, health_2004, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2004) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2006 <- full_join(pequiv_2006, health_2006, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2006) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2008 <- full_join(pequiv_2008, health_2008, by ="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2008) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2010 <- full_join(pequiv_2010, health_2010, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2010) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2012 <- full_join(pequiv_2012, health_2012, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2012) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2014 <- full_join(pequiv_2014, health_2014, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2014) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2016 <- full_join(pequiv_2016, health_2016, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2016) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2018 <- full_join(pequiv_2018, health_2018, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2018) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

merge_2020 <- full_join(pequiv_2020, health_2020, by="pid") %>% 
  select(cid.x, pid, syear.x, i11102, d11106, mcs,p11101)

colnames(merge_2020) <- c("hh_id", "person_id", "year", "income", "hh_size", 
                          "mcs","life_sat")

# adjust for inflation and put all prices in 2023 prices using the following
# website - CHECK THIS IF SHOULD BE DONE DIFFERENTLY OR AUTOMATICALLY###
...
###SAVE RESULTS####
df_all <- list(merge_2002, merge_2004, merge_2006, merge_2008, merge_2010, 
               merge_2012, merge_2014, merge_2016, merge_2018, merge_2020)

processed_soep <- bind_rows(df_all) %>%
  filter(!is.na(mcs)) # 226,251
