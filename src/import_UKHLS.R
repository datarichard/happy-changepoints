...
#Adjust income via OECD household method of divided by square root - CHECK THIS AS DIFFERENT TO WHAT WAS DONE IN HILDA PAPER#
W_1_merge <- W_1_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.52/sqrt(hh_size))
W_2_merge <- W_2_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.48/sqrt(hh_size))
W_3_merge <- W_3_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.41/sqrt(hh_size))
W_4_merge <- W_4_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.37/sqrt(hh_size))
W_5_merge <- W_5_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.34/sqrt(hh_size))
W_6_merge <- W_6_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0))) %>% mutate(income_ann = hh_net_income_month*12*1.32/sqrt(hh_size))
W_7_merge <- W_7_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.32/sqrt(hh_size))
W_8_merge <- W_8_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.31/sqrt(hh_size))
W_9_merge <- W_9_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.28/sqrt(hh_size))
W_10_merge <- W_10_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.25/sqrt(hh_size))
W_11_merge <- W_11_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.22/sqrt(hh_size))
W_12_merge <- W_12_merge %>% filter(!(hh_net_income_month<0), !(is.na(hh_net_income_month)) !(is.na(mcs)),!(mcs<0)) %>% mutate(income_ann = hh_net_income_month*12*1.21/sqrt(hh_size))
